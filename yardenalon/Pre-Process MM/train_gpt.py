...
class CustomDatasetWithTensor(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        numerical_input = self.numerical_inputs[idx]
        numerical_output = self.numerical_outputs[idx]
...
def train_model(model, dataset, batch_size=32, epochs=50, learning_rate=0.001, use_cuda=True, save_interval=5):
    ...
    for epoch in range(epochs):
        ...
        for i, ((images, num_inputs), num_outputs) in enumerate(dataloader):
            ...
            if (i + 1) % 3 == 0:
                print(f"Iteration {i+1}, Loss: {loss.item():.8f}, Time taken for last 100 images: {time.time() - start_iter_time:.2f} seconds")
        ...
        print(f"Time taken for epoch {epoch + 1}: {time.time() - start_epoch_time:.2f} seconds")
