    ...
    def prepare_data(self, paths, output_type):
        ...
        for idx, p in enumerate(paths):
            if idx == 0:
                ...
                df_data_train["frame"] = df_data_train["frame"].str.replace('./datasets', '/home/roblab20/Documents/Allsihgt_proj/datasets')
            else:
                df_data_test = pd.read_json(p).transpose()
                df_data_test["frame"] = df_data_test["frame"].str.replace('./datasets', '/home/roblab20/Documents/Allsihgt_proj/datasets')
